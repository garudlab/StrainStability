...
if cohort == "Poyet" and db_type == "isolate":
    ...
    host_samples = list(pd.read_csv("%s/host_samples/poyet_%s.txt" % (metadata_directory,host),header=None)[0])

elif cohort == "Poyet" and db_type == "standard":
    data_directory = "/u/scratch/r/rwolff/Poyet_midas_output/merged_midas_output/%s_standard/" % host
    ...
    midas_directory = "/u/project/ngarud/Garud_lab/midas_db_v1.2/"
    host_samples = list(pd.read_csv("%s/host_samples/suez_%s.txt" % (metadata_directory,host),header=None)[0])
    
analysis_directory = os.path.expanduser("~/ben_nandita_hmp_analysis/")
scripts_directory = os.path.expanduser("~/strain_stability_revisions/strainstability/microbiome_evolution/")
