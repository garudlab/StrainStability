...
parser.add_argument("--fraction-covered", type=float, help="Fraction of timepoints with sufficient coverage",default=0.3)
parser.add_argument("--fraction-var", type=float, help="Fraction of timepoints with sufficient # snvs",default=0.2)
...
snps_directory="%s/snps" % parse_midas_data.data_directory

sys.stderr.write("Loading core genes...\n")
all_core_genes = core_gene_utils.parse_core_genes(species_name)
...
host_samples = config.host_samples
...
while final_line_number >= 0:
    ...
    snp_samples, allele_counts_map, passed_sites_map, final_line_number = parse_midas_data.parse_snps(species_name,debug=debug, allowed_variant_types=set(['1D','2D','3D','4D']),chunk_size=chunk_size, initial_line_number=final_line_number, allowed_genes=non_shared_genes,allowed_samples=host_samples)
    ...
    for gene_name in allele_counts_map.keys():
        ...
        for var_type in allele_counts_map[gene_name].keys():
            ...
            for snp_idx in xrange(0,len(locations)):
                ...
                ## exclude sites with less than 30% of samples having sufficient (5x+) coverage. 
                ## downstream, coverage of 10x+ will be used to actually assess allele frequencies, 
                ## but this code is used to exclude low-quality sites 
                insufficient_coverage = (good_idxs.sum() < fraction_covered*depths.shape[1])
                
                masked_freqs = freqs[snp_idx][good_idxs]
                
                ### exclude sites with frequency less than 5% in all samples
    ...
    ## not used in this analysis - debugging purposes
